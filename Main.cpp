 ...
 * @var key int - key pressed (global so helpers can exit)
...
bool  draw(cv::Mat &frame, const std::vector<cv::Vec3f> &circles, int nframe);
 ...
 * @param nframe - The current frame number.
 *
...
bool draw(cv::Mat &frame, const std::vector<cv::Vec3f> &circles, int nframe)
{
    try {
        ...
        std::string txt = "COINS: " + std::to_string(circles.size()) +
                          "  |  FRAME: " + std::to_string(nframe);
 ...
 * The loop continues until the user presses the 'q' key or there are no more frames to read.
...
int first_function()
{
    const std::string videoFile = "video1.mp4";   // alter if necessary
    ...
    while (key != 'q')
    {
        ...
        draw(frame, circles, currentFrame);
 ...
 * @details
 * This is the main function of the program.
 * It calls the first_function to start the coin detection process.
